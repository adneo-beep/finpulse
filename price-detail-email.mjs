 ...
 * 실행: node price-detail-email.mjs            → 전체(Sheet1+2+3), 금요일용
 *       node price-detail-email.mjs --mode=thu → 한국부동산원 발표분만(Sheet1+3), 목요일용
 */
...
const MODE = process.argv.includes('--mode=thu') ? 'thu' : 'full';

const RBONE_KEY      = process.env.RBONE_KEY;
...
const APP_PASS  = process.env.NAVER_APP_PASS;

for (const [k, v] of Object.entries({ RBONE_KEY, MAIL_TO, NAVER_APP_PASS: APP_PASS })) {
  if (!v) { console.error(`❌ 필수 환경변수 누락: ${k} (.env 또는 GitHub Secrets 확인)`); process.exit(1); }
}
...
// 이메일 발송
// ────────────────────────────────────────────────────────────
async function sendEmail(excelBuf, weekLabels, weekDates, mode) {
  ...
  const subjectTag = mode === 'thu' ? ' (한국부동산원 발표분)' : ' (전체)';
  const subject = `[부동산 주간동향]${subjectTag} ${latestWeek} (${latestDate} 기준)`;

  const sheetRows = [
    { name: 'Sheet 1', desc: '주택시장동향 총괄 (매매/전세 20개 시도 + KB 심리지수)', included: true },
    { name: 'Sheet 2', desc: 'KB 소비자심리지수 상세 (지역별 세부 구성 비율)', included: mode !== 'thu' },
    { name: 'Sheet 3', desc: '한국부동산원 아파트 매매 시군구 세부 (약 213개 지역)', included: true },
  ].filter(s => s.included);

  const noteLine = mode === 'thu'
    ? '<p style="margin:12px 0 0;font-size:12px;color:#555">KB 소비자심리지수(Sheet 2)는 금요일 발표 후 별도 메일로 발송됩니다.</p>'
    : '<p style="margin:12px 0 0;font-size:12px;color:#555">한국부동산원 발표분(Sheet 1, 3)도 최신 데이터로 갱신하여 함께 첨부합니다.</p>';
...
<div style="font-family:맑은 고딕,sans-serif;font-size:13px;color:#222;max-width:620px">
  <div style="background:#1a3a6b;color:#fff;padding:14px 20px;border-radius:6px 6px 0 0">
    <h2 style="margin:0;font-size:16px">📊 주간 주택시장 동향 리포트${subjectTag}</h2>
  ...
  <div style="background:#f8f9fc;padding:16px 20px;border:1px solid #dde">
    ...
    <table style="border-collapse:collapse;font-size:12px;width:100%">
      ...
      <tr style="background:${i % 2 === 0 ? '#fff' : '#f5f7fb'}">
        <td style="padding:5px 10px;border-bottom:1px solid #eee">${s.name}</td>
        <td style="padding:5px 10px;border-bottom:1px solid #eee">${s.desc}</td>
      </tr>`).join('')}
    </table>
  ...
  const filenameTag = mode === 'thu' ? '_REB' : '';
  const filename = `주택시장동향${filenameTag}_${latestWeek}_${latestDate.replace('.','')}.xlsx`;
...
async function main() {
  ...
  console.log(`📊 Excel 생성 중... (모드: ${MODE === 'thu' ? '목요일 - 한국부동산원 발표분' : '금요일 - 전체'})`);
  ...
  console.log(`✅ Excel 완성 (${(excelBuf.length/1024).toFixed(0)}KB, 시트 ${sheetCount}개)`);

  await sendEmail(excelBuf, weekLabels, weekDates, MODE);
  console.log('🎉 완료');
}
